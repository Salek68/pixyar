...
class ProcessAllApis implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            if ($responsePosts->failed()) {
                return [
                    'step' => 'posts_api_failed',
                    'status' => $responsePosts->status(),
                    'body' => $responsePosts->body()
                ];
            }
            ...
            if ($posts->count() == 0) {
                return ['step' => 'no_posts_after_save'];
            }
            ...
            InstagramProfileSnapshot::create([
